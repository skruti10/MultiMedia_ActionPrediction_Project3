...
from GetGIFsOutput import convert_images_to_gif
...
class GeneratorModel:
    ...
    def define_graph(self):
        ...
        with tf.name_scope('generator'):
            ...
            with tf.name_scope('data'):
                ...
                print('GENERATOR: batch_size_train: ',self.batch_size_train)
                print('GENERATOR: batch_size_test: ',self.batch_size_test)
        ...
        if global_step % c.IMG_SAVE_FREQ == 0:
            ...
            for scale_num in range(self.num_scale_nets):
                ...
                for i, img in enumerate(gt_frames):
                    ...
                    #matching = numpy.all(sknorm_img >= 1, axis=2)
                    #sknorm_img[matching, :] -= 0.5
    ...
    def test_batch(self, batch, global_step, num_rec_out=1, save_imgs=True):
        ...
        print ('-' * 30)
        print('Testing with GlobalStep ', global_step)
        print ('Testing:')
        ...
        for rec_num in range(num_rec_out):
            ...
            #print('working_gt_frames', working_gt_frames)
        ...
        if save_imgs:
            ...
            #print('test dir: ', c.TEST_DIR)
            sourceFolderForGIF = ''
            newTestDir = -1
            new_test_dir = c.TEST_DIR.split("/")
            count = len(new_test_dir)

            if new_test_dir[count-1] != 'Test':
                temp_directory = os.path.join(c.IMG_SAVE_DIR, 'Tests/'+str(new_test_dir[count-1])+'/Step_' + str(global_step))
                #print('temp_directory', temp_directory)
                rnd_int = random.randint(0,100)
                if not os.path.exists(temp_directory):
                    c.get_dir(temp_directory)
                else:
                    temp_directory = os.path.join(temp_directory+'_'+str(rnd_int))
                    c.get_dir(temp_directory)
            ...
            for pred_num in range(len(input_frames)):
                if newTestDir == -1:
                    pred_dir = c.get_dir(os.path.join(c.IMG_SAVE_DIR, 'Tests/Step_' + str(global_step), str(pred_num)))
                    sourceFolderForGIF = os.path.join(c.IMG_SAVE_DIR, 'Tests/Step_' + str(global_step))
                else:
                    pred_dir = c.get_dir(os.path.join(temp_directory, str(pred_num)))
                    #print('pred_dir: ', pred_dir)
                    sourceFolderForGIF = temp_directory
            ...
            convert_images_to_gif(sourceFolderForGIF)

        print ('-' * 30)
        c.GIF_SRC_FOLDER = sourceFolderForGIF
