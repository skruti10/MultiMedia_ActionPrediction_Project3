...
class AVGPrediction:
    def __init__(self, num_steps, model_load_path, num_test_rec):
        ...
        if c.ADVERSARIAL:
            print('Initiating discriminator...')
        ...
        print('Initiating other variables...')
        ...
        if model_load_path is not None:
            ...
            ckpt = tf.train.get_checkpoint_state(c.MODEL_SAVE_DIR)
    ...
    def train(self):
        """
        Runs a training loop on the model networks for a number of steps
        """
        for i in range(self.num_steps):
            #check if we Adversarial network will be used for training
            if c.ADVERSARIAL:
                # update discriminator
                # get a batch of random images from the saved clips
                batch = get_train_batch()
                print('Train the discriminator...')
                self.d_model.train_step(batch, self.g_model)

            # update generator
            # get a batch of random clips from the saved clips. The batch size is set as 8
            batch = get_train_batch()
            ...
            # save the models as per the model frequency assigned in Constants.
            # this can also be passed as a parameter
            ...
            # currently the network is processed till these steps.
            if self.global_step % 10 == 0:
                print('Processed %d steps' % (self.global_step))

            # test generator model.
            # testing after few iterations can give idea about how the images are being generated.
...
def main():
    ...
    runner = AVGPrediction(num_steps, load_path, num_test_rec)
    if test_only:
        runner.test()
    else:
        runner.train()
