    ...
    for clip_num, ep_dir in enumerate(ep_dirs):
        ...
        if(len(ep_frame_paths) > 4):
            ...
            for frame_num, frame_path in enumerate(clip_frame_paths):
                temp_Height, temp_Width = c.get_train_frame_dims_1(frame_path)

                '''clips = np.empty([num_clips,
                    temp_Height*rescaleFactor2,
                    temp_Width*rescaleFactor1,
                    (3 * (c.HIST_LEN + num_rec_out))])'''

                '''if temp_Width != 160 or temp_Height != 210:
                    temp_image = cv2.imread(frame_path)
                    res = cv2.resize(temp_image,(c.FULL_WIDTH, c.FULL_HEIGHT), interpolation = cv2.INTER_LINEAR)
                    frame = imread(res, mode='RGB')                    
                else:
                ...
                if temp_Width != 160 or temp_Height != 210:
                    #print('in resize')
                    frame = scipy.misc.imresize(frame, (c.FULL_HEIGHT, c.FULL_WIDTH))
...
def get_train_batch():
    ...
    print('train dir clips', c.TRAIN_DIR_CLIPS)
