...
# get the test frames dimensions - Width and Height  from Test directory
...
def get_train_frame_dims_1(imagePath):
    img = imread(imagePath, mode='RGB')
    shape = np.shape(img)

    return shape[0], shape[1]

# get the test frames dimensions - Width and Height  from Train directory
def get_train_frame_dims():
    print('gettrainframes:', TRAIN_DIR)
...
# the method will be called when we try to set a new test directory in Test_Mode while running main_prediction.py
...
# root directory for all data - this is present inside the Code directory.
DATA_DIR = get_dir('data/')
# directory of unprocessed training frames - we have intialized it with the Human_Actions directory.
TRAIN_DIR = os.path.join(DATA_DIR, 'Human_Actions/Train/')
# directory of unprocessed test frames
TEST_DIR = os.path.join(DATA_DIR, 'Human_Actions/Test/')
# Directory of processed training clips.
# hidden so finder doesn't freeze w/ so many files. DON'T USE `ls` COMMAND ON THIS DIR!
TRAIN_DIR_CLIPS = get_dir(os.path.join(DATA_DIR, 'Human_Actions/Clips/'))
...
# the height and width of the full frames to test on. Set in main_prediction.py or process_data.py main.
FULL_HEIGHT = 210 
FULL_WIDTH = 160 
# the height and width of the patches to train on
TRAIN_HEIGHT = TRAIN_WIDTH = 32

# save all the GIFs in the folder path as generated during the test.
GIF_SRC_FOLDER = ''

##
# Save the output in specific directories.
##
...
SAVE_DIR = get_dir('data/Human_Actions/Save/')
...
MODEL_SAVE_FREQ = 3000   # how often to save the model, in # steps
